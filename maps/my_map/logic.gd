...
func get_spawn_point_position_for_team(teamNumber):
	if teamNumber == 1:
		$Team1SpawnPoints.get_children().pick_random()
		var spawnPoint =  $Team1SpawnPoints.get_children().pick_random()
		return spawnPoint.global_position
	elif teamNumber == 2:
		var spawnPoint = $Team2SpawnPoints.get_children().pick_random()
		return spawnPoint.global_position
	else:
		return Vector3.ZERO
